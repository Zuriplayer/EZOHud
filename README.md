...
- Custom Group Search panel that hides ESO's native on-screen Activity Finder status tracker and shows status, selected role, activity name, queue time, estimate, and ready-check counts when the ESO API exposes them.
...
- Custom Group Search: enable the custom Activity Finder status panel, allow movement, and adjust scale. The panel replaces only the small HUD status tracker, not the full finder window.
...
- Test Custom Group Search while queued for a dungeon or other Activity Finder activity, during ready check, and after queue completion to confirm the native tracker hides, role/activity/time text updates, and the custom panel disappears outside HUD scenes.
