...
- Custom Action Bars are informational only. When requested, they can hide ESO's native HUD action bar visually for normal primary/backup weapon bars; temporary and mechanic-specific native bars remain visible. They do not cast abilities, change weapons, use quickslot items, trigger keybinds, or automate rotations. Effect timers, stack counts, backup-slot visibility, ultimate percentage/current-cost values, charge bar, and quickslot cooldowns follow the native data exposed by ESO; the delayed Subterranean Assault and Deep Fissure shalk cycles additionally use six- or nine-second predicted timers because ESO does not consistently expose those phases through the slot timer API. Other slots without native timer data remain blank. Key labels are display-only and follow ESO's current bindings when enabled.
