...
- Parallel horizontal Custom Action Bars with generic weapon-category icons, inactive weapon-icon hiding, slot-use flash animation, large centered white native effect timers, orange upper-right stack counts, a configurable proportional timer-warning threshold, optional scalable native key labels below the icons, configurable shared timer-bar color, icon size up to 96 px, spacing, inactive-bar alpha, dimmed-slot alpha, ultimate power/cost text with not-ready dimming, and saved global dimming choices for weapon, ability 1-5, and ultimate slots. Timed ability slots remain fully visible even on the inactive row.
...
- Custom Action Bars: enable visual copies of ability bars, optionally hide ESO's native HUD action bar, choose visible rows, move the paired action-bar block and active quickslot indicator independently, adjust icon size/spacing/alpha, toggle large centered native action-slot effect timers and orange upper-right stack counts, choose the shared timer-bar color and proportional warning threshold, choose key labels off/auto/keyboard/gamepad below the icons, and choose globally dimmed logical slots. The active quickslot icon mirrors ESO's cooldown data with a vertical refill and remaining-time label.
...
- Custom Action Bars are informational only. When requested, they can hide ESO's native HUD action bar visually, but they do not cast abilities, change weapons, use quickslot items, trigger keybinds, or automate rotations. Effect timers, stack counts, and quickslot cooldowns are shown only when ESO exposes native slot data; slots without native timer data remain blank. Key labels are display-only and follow ESO's current bindings when enabled.
...
- Test Custom Action Bars with main, backup, both, and active-only modes. Confirm the master enable toggle immediately enables dependent settings in both Settings > EZO and the standalone LAM panel without leaving them visually greyed out; both rows stay parallel and move together as one block; weapon icons update after weapon swap; only the active weapon icon remains visible with the purple frame; active-row highlighting follows bar swaps; used slots flash when activated; dimmed slot choices apply to both rows except when a native timer is currently active; native timers appear as large centered white numbers; stack counts appear in orange at the upper-right; inactive-row ability slots with active timers remain fully visible; the shared timer-bar color and warning threshold apply; key-label Off keeps the compact layout while Auto/Keyboard/Gamepad place the native labels below each icon without overlap; the active quickslot icon still moves independently, shows item count when ready, switches to remaining time during potion/drink cooldown, and refills vertically; ultimate shows current/cost and dims before it is ready; and the native action bar hides/restores with its toggle.
