...
- Currency icons use ESO's colored keyboard textures in both input modes, with ESO's gamepad textures as a fallback when a keyboard texture is unavailable.
...
- Activate and dismiss the mouse cursor while the Currency Information Panel is visible; confirm its currency icons keep their colors in both states.
