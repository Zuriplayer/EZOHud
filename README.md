...
- Custom Action Bars are informational only. When requested, they can hide ESO's native HUD action bar visually, but they do not cast abilities, change weapons, use quickslot items, trigger keybinds, or automate rotations. Effect timers, stack counts, backup-slot visibility, ultimate percentage/current-cost values or charge bar, and quickslot cooldowns follow only the native slot data exposed by ESO; slots without native timer data remain blank. Key labels are display-only and follow ESO's current bindings when enabled.
